...
import com.example.ohxinli.databinding.ActivityBookingRequestBinding
import com.google.firebase.firestore.FirebaseFirestore

class booking_request : AppCompatActivity() {
    ...
    private fun getBookingData() {
        var userIc: String
        var name = ""
        val requestList: ArrayList<BookingClass3> = ArrayList()
...
//                    var userIC = db.collection("User").whereEqualTo("ic_no", "$userIc").get().toString()
//
//                    db.collection("User").get()
//                        .addOnSuccessListener { doc ->
//                            for(document in doc){
//                                name = document.get("name").toString()
//                                val useric = document.get("ic_no").toString()
//
//                                if(useric == userIC){
//                                    name = document.get("name").toString()
//                                    val request = BookingClass3(bookingDate, bookingTime, session.toInt(), name, userIc)
//                                    requestList.add(request)
//
//                                    var bookingRequestRecyclerView = binding.requestList
//                                    bookingRequestRecyclerView.layoutManager = LinearLayoutManager(this)
//                                    bookingRequestRecyclerView.adapter = RequestAdapter(requestList)
//                                }
//                            }
//
//                        }
...
//
//                                name = document.get("name").toString()
                    var request = BookingClass3(bookingDate, bookingTime, session.toInt(), name, userIc)
